...
### `docs/*.md` planning docs
`docs/VMS_DEVELOPMENT_PLAN.md`, `docs/TRIP_IMPLEMENTATION_GUIDE.md`, `docs/TRIP_SYSTEM_FINAL.md`, `docs/TRIP_DYNAMIC_FIELDS_GUIDE.md`, `docs/RECURRING_TRIPS_SIMPLE.md`, `docs/MIGRATION_CONSOLIDATION.md`, `docs/FRONTEND_UPDATES.md` are historical design/planning notes written at different stages of the project. They're useful for *why* a feature exists but several describe designs (e.g. weekly recurrence patterns, `TripService` classes, tables like `trip_recurring_series`/`trip_series_occurrences`) that were later simplified or never actually implemented. Treat `database/migrations/` and the current models as the source of truth over these docs, and verify anything you read there against the actual code before relying on it. (`docs/` also holds a few other planning notes not listed individually here — `ATTENDANCE_SYSTEM_PLAN.md`, `GOOGLE_GEOCODING_IMPLEMENTATION.md`, `PROJECT_IMPROVEMENTS.md`, `LARAVEL13_INERTIA3_UPGRADE_PLAN.md` — same caveat applies.)
